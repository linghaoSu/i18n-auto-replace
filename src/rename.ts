...
async function onRenameDirectory(oldUri: Uri, newUri: Uri) {
  const currentDirFiles = await readDirectory(newUri)

  currentDirFiles.forEach(async (item) => {
    const [fileName] = item

    const newDirFile = Uri.joinPath(newUri, fileName)
    const oldDirFile = Uri.joinPath(oldUri, fileName)
    onRenameFile(oldDirFile, newDirFile)
  })
}
